frappe.ui.form.on("Bulk WhatsApp Message", {
  refresh: function (frm) {
    ...
    if (frm.doc.docstatus === 1 && frm.doc.status != "Draft") {
            ...
            if (r.message) {
              ...
              let html = `
                                ...
                                <div class="mt-2">
                                    ...
                                    <span class="badge badge-danger ml-2">Failed: ${
                                      progress.failed
                                    }</span>
                                    <span class="badge badge-warning ml-2">Queued: ${
                                      progress.queued
                                    }</span>
                                    <span class="badge badge-info ml-2">Total: ${
                                      progress.total
                                    }</span>
                                </div>
                            `;
      ...
      // Add retry button
      frm
        .add_custom_button(__("Retry Failed Messages"), function () {
          frappe.call({
  ...
  validate: function (frm) {
    if (
      frm.doc.recipient_type == "Individual" &&
